...
const toWordHtml = (text: string, title: string) => {
  ...
  const tableHtml =
    tableRows.length >= 2
      ? (() => {
          const headers = tableRows[0].map((cell) => `<th>${safeHtml(cell)}</th>`).join('');
          const body = tableRows
            .slice(1, 18)
            .map(
              (row) =>
                `<tr>${row.map((cell) => `<td>${safeHtml(cell)}</td>`).join('')}</tr>`,
            )
            .join('');
...
body{font-family:'Segoe UI',Calibri,Arial,sans-serif;font-size:11.5pt;line-height:1.6;color:#0f172a;margin:0;padding:0;background:#f8fafc;}
.doc{max-width:900px;margin:0 auto;background:#fff;box-shadow:0 16px 42px -28px rgba(15,23,42,.45);}
.hero{background:linear-gradient(135deg,#0b3b79,#0f57a6);color:#fff;padding:30px 34px 28px;}
.hero h1{font-size:27pt;line-height:1.12;margin:0;}
.hero p{margin:8px 0 0;font-size:10pt;opacity:.86;}
.content{padding:28px 34px 34px;}
h2{font-size:14.5pt;line-height:1.3;margin:16px 0 8px;color:#0b3b79;border-bottom:1px solid #dbeafe;padding-bottom:4px;}
p{margin:0 0 10px;}
ul{margin:0 0 12px 18px;padding:0;}
li{margin:0 0 6px;}
table{width:100%;border-collapse:collapse;margin:14px 0 16px;font-size:10.5pt;}
th{background:#0b3b79;color:#fff;padding:8px;text-align:left;font-weight:600;border:1px solid #c7d2fe;}
td{padding:8px;border:1px solid #dbe2ea;}
tbody tr:nth-child(even) td{background:#f8fbff;}
</style></head><body><div class="doc"><div class="hero"><h1>${safeTitle}</h1><p>Generated by Atlas • ${new Date().toISOString()}</p></div><div class="content">${tableHtml}${bodyParts.join('')}</div></div></body></html>`;
};
